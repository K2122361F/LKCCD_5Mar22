...
def index():
    if request.method=="POST":
        ...
        s1="The score of client default base on decision tree is " +str(pred1)
        
        
        model2=joblib.load("CCD_Reg")
        pred2=model2.predict([[income, age, loan]])
        s2="The score of client default base on logistic regression is " +str(pred2)
        
    
        model3=joblib.load("CCD_NN")
        pred3=model3.predict([[income, age, loan]])
        s3="The score of client default base on neural network model is " +str(pred3)
        
        model4=joblib.load("CCD_RF")
        pred4=model4.predict([[income, age, loan]])
        s4="The score of client default base on random forest model is " +str(pred4) 
        
        
        model5=joblib.load("CCD_GB")
        pred5=model5.predict([[income, age, loan]])
        s5="The score of client default base on gradient boosting model is " +str(pred5) 
